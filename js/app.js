(function() {
  ...
  let current = 0;
  //click listener to change custom variables
  document.body.addEventListener("click", () => {
    ...
    document
      .querySelector(":root")
      .style.setProperty("--theme-1-bg", bgList[current]);
    document
      .querySelector(":root")
      .style.setProperty("--theme-1-color", colorList[current]);
    current++;
    const tl = gsap.timeline();
    tl.fromTo(
      "#js-ripple",
      { width: "0", height: "0", opacity: 1 },
      { width: "62px", height: "62px", opacity: 0, duration: 0.6, stagger: 0.3 }
    );
  });
  //Intro Animation
  const tl = gsap.timeline();
